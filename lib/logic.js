...
async function TransferMedicationConsentToProvider(tx) {
    ...
    if (asset.sharedWith) {
        if (asset.sharedWith.indexOf(providerId) > -1) {
            console.log('Asset ' + tx.asset.detail + ' is already shared with ' + tx.provider.firstName);
        }
        else {
            // add provider ID to sharedWith array
            asset.sharedWith.push(providerId);
        }
    }
    else {
        console.log('You are sharing this item for the first time');
        asset.sharedWith = [providerId];
    }
    // Update the registry
    ...
    let event = getFactory().newEvent('org.cc.patientdatanetwork', 'ItemShared');
    event.asset = asset;
